...
class RiskManager:
    ...
    def calculate_stop_loss(self, entry_price: float, confidence: str) -> float:
        ...
        if "Highest" in confidence:
            sl_pct = self.default_stop_loss_pct * 0.8  # Tighter SL for highest confidence
        elif "High" in confidence:
            sl_pct = self.default_stop_loss_pct
        elif "Medium" in confidence:
            sl_pct = self.default_stop_loss_pct * 1.2  # Wider SL for medium confidence
        else:
            sl_pct = self.default_stop_loss_pct * 1.5  # Much wider for low confidence
    ...
    def calculate_take_profit(self, entry_price: float, confidence: str, signal_tags: List[str]) -> float:
        ...
        """
        base_tp_pct = self.default_take_profit_pct
        
        # Adjust TP based on confidence
        if "Highest" in confidence:
            tp_pct = base_tp_pct * 1.5  # Higher targets for highest confidence
        elif "High" in confidence:
            tp_pct = base_tp_pct
        elif "Medium" in confidence:
            tp_pct = base_tp_pct * 0.8  # Lower targets for medium confidence
        else:
            tp_pct = base_tp_pct * 0.6  # Much lower for low confidence
        
        # Adjust based on signal type
        if any(tag.lower() in ['fusion', 'confluence'] for tag in signal_tags):
            tp_pct *= 1.3  # Higher targets for confluence signals
        elif any(tag.lower() in ['gamma', 'pin'] for tag in signal_tags):
            tp_pct *= 1.1  # Slightly higher for gamma signals
...
class TradeExecutorEngine:
    ...
    def update_positions(self, end_index: Optional[int] = None) -> Dict[str, int]:
        ...
        print(f"\n📈 UPDATING POSITIONS ({start_index} to {end_index})")
        print("="*50)
        
        # Process each tick of market data
        for idx in range(start_index, end_index):
            ...
            self.current_timestamp = row['timestamp'].strftime('%Y-%m-%d %H:%M:%S')
            ...
            for position in self.active_positions:
                should_close = position.update_current_price(
                    self.current_price, idx, self.current_timestamp
                )
        ...
        if end_index >= len(self.market_data) - 1:
            ...
            for position in remaining_positions:
                position.force_close(self.current_price, self.current_timestamp, self.current_index)
                self.active_positions.remove(position)
                self.closed_trades.append(position)
                positions_closed += 1
